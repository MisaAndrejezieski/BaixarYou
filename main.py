...
def download_media(url):
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            # Pegar título e verificar extensão
            title = info_dict.get('title', 'Desconhecido').replace(" ", "_")
            ext = info_dict.get('ext', 'unknown')  # Obter extensão

            # Ajustar para extensão padrão caso não exista ou seja inválida
            if ext == 'unknown':
                if 'image' in info_dict.get('_type', ''):  # Detectar imagem
                    ext = 'jpg'  # Extensão padrão para imagens
                else:
                    ext = 'mp4'  # Extensão padrão para vídeos
