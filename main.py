...
class VideoDownloader(ctk.CTk):
    ...
    def validar_url(self, url: str) -> bool:
        padroes = [
            ...
            r'^https?://(www\.)?(youtu\.be)/',  # YouTube short links
        ]
    ...
    def download_video(self, url: str) -> None:
        try:
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                self.after(0, self.status_label.configure, {"text": "📥 Obtendo informações do vídeo..."})
                ydl.download([url])
    ...
    def atualizar_progresso(self, d: dict) -> None:
        ...
        try:
            if d['status'] == 'downloading':
                ...
                self.after(0, self.atualizar_barra, valor_float, percentual, velocidade)
            
            elif d['status'] == 'finished':
                self.after(0, self.status_label.configure, {"text": "✨ Processando vídeo (convertendo para MP4)..."})
                
        except Exception as e:
            # Evita que erro no progresso quebre o download
