    ...
    def download_video(self, url: str) -> None:
        try:
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                self.after(0, lambda: self.status_label.configure(text="📥 Obtendo informações do vídeo..."))
                ydl.download([url])
            ...
            # Erro
            self.after(0, lambda: self.download_erro(str(e)))
    ...
    def atualizar_progresso(self, d: dict) -> None:
        ...
        try:
            if d['status'] == 'downloading':
                ...
                self.after(0, lambda: self.atualizar_barra(valor_float, percentual, velocidade))
            
            elif d['status'] == 'finished':
                self.after(0, lambda: self.status_label.configure(text="✨ Processando vídeo (convertendo para MP4)..."))
