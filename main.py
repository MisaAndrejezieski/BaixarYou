...
class VideoDownloader(ctk.CTk):
    ...
    def create_widgets(self):
        ctk.CTkLabel(self, text="Baixar Vídeos", font=("Arial", 26, "bold")).pack(pady=20)

        self.url_entry = ctk.CTkEntry(
            self, width=500, height=45,
        ...
        self.download_btn = ctk.CTkButton(
            self, text="Baixar Vídeo", command=self.start_download,
            width=250, height=45, font=("Arial", 14, "bold")
        )
        self.download_btn.pack(pady=15)

        self.exibir_btn = ctk.CTkButton(
            self, text="▶️ Exibir Vídeo", command=self.exibir_video,
            width=250, height=45, font=("Arial", 14, "bold"),
            state="disabled"
        )
